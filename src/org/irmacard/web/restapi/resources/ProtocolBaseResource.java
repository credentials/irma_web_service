...
public abstract class ProtocolBaseResource extends ServerResource  {
	...
	public Representation handleGet() {
		...
		if (step != null) {
			if (step.equals("qr")) {
				...
				String qrURL = getBaseURL() + path.substring(0, path.lastIndexOf('/')+1) + "0";
	...
	public String createQRResponse(String id) {
		...
		qrr.status_url = getReference().getPath() + "/" + id + "/status";
	...
	public String makeResponseURL(String id, int step) {
		...
		if (reqID == null) {
			return getBaseURL() + path + '/' + id + '/' + Integer.toString(step);
		}
